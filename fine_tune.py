...
CORRUPTIONS_DS = {
    ...
    'dermamnist' : {
        'pixelate': Pixelate(severity_params=[0.8,0.7, 0.5, 0.40, 0.30, 0.25]),
        'jpeg_compression' : JPEGCompression(severity_params=[100, 20, 15, 10, 7]),
        'gaussian_noise' : GaussianNoise(severity_params=[0.012, 0.04,0.08, 0.12, 0.18, 0.26]),
        'brightness_up' : Brightness(severity_params=[1.0,1.1, 1.2, 1.3, 1.4, 1.5]),
        'brightness_down' : Brightness(severity_params=[0.9, 0.8, 0.7, 0.6, 0.5,0.4]),
    },
    'pneumoniamnist' : {
...
def fine_tune_model(model:str, data_name: str, n_classes: int, batch_size: int, gpus:int, epochs:int, corruption:bool):
    ...
    subprocess.call(["chmod", "+x", './training/train.sh'])

    
    commands = ['training/train.sh', f'{gpus}',
